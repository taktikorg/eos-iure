...
[![code coverage](https://zjonsson.github.io/node-@taktikorg/eos-iure/badge.svg)](https://zjonsson.github.io/node-@taktikorg/eos-iure/) 

[npm-image]: https://img.shields.io/npm/v/@taktikorg/eos-iure.svg
[npm-url]: https://npmjs.org/package/@taktikorg/eos-iure
[travis-image]: https://api.travis-ci.org/ZJONSSON/node-@taktikorg/eos-iure.png?branch=master
[travis-url]: https://travis-ci.org/ZJONSSON/node-@taktikorg/eos-iure?branch=master
[downloads-image]: https://img.shields.io/npm/dm/@taktikorg/eos-iure.svg
[downloads-url]: https://npmjs.org/package/@taktikorg/eos-iure
[coverage-image]: https://3tjjj5abqi.execute-api.us-east-1.amazonaws.com/prod/node-@taktikorg/eos-iure/badge
[coverage-url]: https://3tjjj5abqi.execute-api.us-east-1.amazonaws.com/prod/node-@taktikorg/eos-iure/url
...
```bash
$ npm install @taktikorg/eos-iure
```
...
fs.createReadStream('path/to/archive.zip')
  .pipe(@taktikorg/eos-iure.Extract({ path: 'output/path' }));
```
...
```js
const zip = fs.createReadStream('path/to/archive.zip').pipe(@taktikorg/eos-iure.Parse({forceStream: true}));
...
If you `pipe` from @taktikorg/eos-iure the downstream components will receive each `entry` for further processing.   This allows for clean pipelines transforming zipfiles into unzipped data.
...
```js
async function main() {
  const directory = await @taktikorg/eos-iure.Open.file('path/to/archive.zip');
...
const @taktikorg/eos-iure = require('./unzip');

async function main() {
  const directory = await @taktikorg/eos-iure.Open.url(request,'http://www2.census.gov/geo/tiger/TIGER2015/ZCTA5/tl_2015_us_zcta510.zip');
...
async function getFile(req, res, next) {
  const directory = await @taktikorg/eos-iure.Open.url(request, googleStorageOptions);
...
async function main() {
  const directory = await @taktikorg/eos-iure.Open.s3(s3Client,{Bucket: '@taktikorg/eos-iure', Key: 'archive.zip'});
  return new Promise( (resolve, reject) => {
    directory.files[0]
      .stream()
      .pipe(fs.createWriteStream('firstFile'))
...
async function main() {
  const directory = await @taktikorg/eos-iure.Open.buffer(buffer);
  console.log('directory',directory);
  // ...
}
...
async function main() {
  ...
  const directory = await @taktikorg/eos-iure.Open.custom(customSource);
  console.log('directory', directory);
  // ...
}
